...
function AddReview({ dao_name, guild_id, uid, slug }) {
  ...
  const submitReview = async (e) => {
    ...
    let public_address = JSON.parse(
      localStorage.getItem("wallet_state")
    )?.address;
    let chain =
      JSON.parse(localStorage.getItem("wallet_state"))?.chain == "Solana"
        ? "sol"
        : "eth";
    chain =
      JSON.parse(localStorage.getItem("wallet_state"))?.chain == "near"
        ? "near"
        : "eth"; //please eedit this and create afucntion
    if (!public_address || public_address?.length < 5) {
    ...
    try {
      ...
      let res = await axios.post(
        `${API}/review-v2/add-review`,
        {
          validation: { uid, slug },
          review: { ...reviewForm, chain, public_address },
        },
...
export async function getServerSideProps(ctx) {
  ...
  let validation = await axios.get(
    `${API}/review-v2/user-validation?uid=${uid}&slug=${target}`
  );
