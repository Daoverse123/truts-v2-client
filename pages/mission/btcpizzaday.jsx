...
import styles from "./pizzaday.module.scss";
...
function Index({ mission }) {
  ...
  const pizzaClaim = async () => {
    ...
    let user = user_res.data.data.user;

    let userId = user._id;
    let username = user.username;

    let res = await axios.get(
      `/api/addpizza?userId=${userId}&username=${username}&city=${selectedCity}`
    );
  ...
  const Pizza = ({ disabled }) => {
    return (
      <div className={styles.pizzaInput}>
...
export const getServerSideProps = async (ctx) => {
  let mission_id = "646889a2b16f108d31b4ab4d";
